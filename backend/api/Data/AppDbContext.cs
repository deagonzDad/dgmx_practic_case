...
namespace api.Data
{
    public class AppDbContext(DbContextOptions<AppDbContext> options) : DbContext(options)
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<User>().HasMany(r=>r.Roles).WithMany(u=>u.Users).UsingEntity(j=>j.ToTable("UserRoles"));
        ...
        public void SeedBasicUsers(AppDbContext context){
            if(!context.Users.Any(u=>u.Username =="admin")){
                ...
                try{
                    var adminRole = context.Roles.Find(1) ?? throw new Exception("Admin role [1] not found");

                    var adminUser = new User{
                        Username = "admin",
                        Email = "example@example.com",
                        Password = PasswordHasher.HashedPassword("admin"),
                        Roles = [adminRole]
                    };
